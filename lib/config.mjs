...
export const upstreamDir = path.join(rootDir, 'vendor', 'guizang-ppt-skill');
...
export function defaultModelConfig() {
  return {
    provider: env.BAILING_PROVIDER || '百灵大模型',
    model: env.BAILING_MODEL || 'Ling-3.0-flash-260716',
    baseUrl: env.BAILING_BASE_URL || 'https://api.ant-ling.com/v1',
    apiKey: env.BAILING_API_KEY || '',
    reasoningEffort: env.BAILING_REASONING_EFFORT || '',
  };
}
...
export function redactSecrets(value, config = {}) {
  ...
  for (const secret of [config.apiKey, env.BAILING_API_KEY].filter(Boolean)) {
