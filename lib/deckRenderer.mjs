...
export const slideTypes = ['cover', 'metrics', 'argument', 'grid', 'process', 'close'];
...
export function normalizePlan(plan = {}, variant = variants[0]) {
  ...
  const slides = [0, 1, 2, 3, 4, 5].map((index) => sourceSlides[index] || {});
  ...
  return {
    ...
    slides: [
      ...
      slides[3].blank ? { type: 'grid', blank: true } :
      {
        type: 'grid',
        kicker: text(slides[3].kicker, 'FOUR DIRECTIONS'),
        title: text(slides[3].title, '四种版本各有用处'),
        cards: normalizeCards(slides[3].cards),
      },
      slides[4].blank ? { type: 'process', blank: true } :
      {
        type: 'process',
        kicker: text(slides[4].kicker, 'WORKFLOW'),
        title: text(slides[4].title, '从一句想法到四份成稿'),
        subtitle: text(slides[4].subtitle, '把排队式生成改成同时探索。'),
        steps: normalizeSteps(slides[4].steps),
      },
      slides[5].blank ? { type: 'close', blank: true } :
      {
        type: 'close',
        kicker: text(slides[5].kicker, 'TAKEAWAYS'),
        title: text(slides[5].title, '最后只需要选一个最喜欢的'),
        body: text(slides[5].body, '速度足够快时，创意选择就会从稀缺变成日常。'),
        takeaways: normalizeList(slides[5].takeaways, ['生成不是终点', '并行带来比较', '选择保留审美'], 3),
      },
    ],
  };
}
  ...
  <div class="canvas-card">
    <div class="split-half">
      <div class="half b-accent">
        <canvas class="ascii-bg" aria-hidden="true"></canvas>
        ${chrome(plan, variant, 6, total, true)}
