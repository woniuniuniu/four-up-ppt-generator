...
export async function reviseDeckPlan({ idea, variant, plan, instruction, settings }) {
  const config = runtimeModelConfig(settings);
  if (!config.apiKey) {
    throw new Error('模型 API key 还没有配置。');
  }
  ...
  const revised = parsed.plan || parsed.deck || parsed;
  const revisedSlides = Array.isArray(revised.slides) ? revised.slides.slice(0, DECK_SLIDE_COUNT) : [];
  if (revisedSlides.length < DECK_SLIDE_COUNT) {
    throw new Error(`模型只返回了 ${revisedSlides.length}/${DECK_SLIDE_COUNT} 页，请换一种更具体的修改要求再试。`);
  }

  const nextPlan = {
    ...normalizedPlan,
    title: stringOrEmpty(revised.deckTitle || revised.deck_title || revised.title) || normalizedPlan.title,
    subtitle: stringOrEmpty(revised.deckSubtitle || revised.deck_subtitle || revised.subtitle) || normalizedPlan.subtitle,
    slides: revisedSlides.map((slide, index) => ({ ...slide, type: slideTypes[index] })),
  };

  const result = normalizePlan(nextPlan, variant);
  for (const [index, slide] of result.slides.entries()) {
    if (!hasUsableSlide(slide, index)) {
      throw new Error(`模型返回的第 ${index + 1} 页不完整，请换一种更具体的修改要求再试。`);
    }
  }
  return result;
}
...
function revisionRequestBody({ idea, variant, config, plan, instruction }) {
  return {
    model: config.model,
    messages: [
      {
        ...
        content: [
          '你是资深中文商业演示策划和信息设计师。',
          '你正在修改一份已经生成好的 HTML PPT。',
          `你必须根据用户整体反馈重写这份 deck 的内容，但仍然输出 ${DECK_SLIDE_COUNT} 页。`,
          '保留原 deck 的视觉系统、版本方向和字段结构；可以调整标题、文案、叙事顺序和每页重点。',
          '只输出一个 JSON object，不要 Markdown，不要解释，不要代码块。',
          '输出对象必须包含 deckTitle、deckSubtitle、slides；slides 数组必须写满全部页面。',
          '如果修改涉及图表，只能使用相对评分、优先级、用户明确提供的数据或清楚标注的观察性判断，不要编造市场份额、营收、增速等外部真实统计。',
        ].join('\n'),
      },
      {
        role: 'user',
        content: buildRevisionPrompt({ idea, variant, plan, instruction }),
      },
    ],
...
function buildRevisionPrompt({ idea, variant, plan, instruction }) {
  return `原始 PPT 想法：
${idea || plan.title}
...
请基于反馈重写整份 deck。你可以只小改，也可以重构表达，但不要改变页数和每页字段类型。
每页仍按下面顺序输出：
0 cover / 1 metrics / 2 argument / 3 chart / 4 grid / 5 process / 6 comparison / 7 roadmap / 8 close

输出格式：
{
  "deckTitle": "整份 deck 标题",
  "deckSubtitle": "一句总副标题",
  "slides": ${fullDeckSchema()}
}

重要：严格输出 JSON object。slides 数组必须写满 ${DECK_SLIDE_COUNT} 个对象。`;
}

function fullDeckSchema() {
  return `[
    ...
    {"index":3,"kicker":"RELATIVE SIGNAL","title":"相对图表页标题","subtitle":"一句说明","chart":{"unit":"/5","note":"说明这是相对评分或用户给出的数据，不是真实外部统计","items":[{"label":"指标一","score":5,"value":"5","unit":"/5","note":"一句说明"},{"label":"指标二","score":4,"value":"4","unit":"/5","note":"一句说明"},{"label":"指标三","score":3,"value":"3","unit":"/5","note":"一句说明"}]}},
